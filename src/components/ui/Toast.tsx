...
import { HiCheckCircle, HiXMark, HiShoppingCart, HiArrowRight } from 'react-icons/hi2';
        ...
        >
          <div className="bg-text-main text-white p-5 rounded-3xl shadow-2xl flex flex-col gap-4 border border-white/10 backdrop-blur-md">
            <div className="flex items-center justify-between">
              ...
              <button onClick={hideToast} className="p-1.5 hover:bg-white/10 rounded-full transition-colors">
            ...
            <div className="bg-white/5 rounded-2xl p-4 flex items-center justify-between border border-white/5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium">{cartCount} {cartCount === 1 ? 'Item' : 'Items'} &bull; ${cartTotal.toFixed(2)}</p>
                </div>
              </div>
              <Link href="/cart" onClick={hideToast}>
                <button className="text-xs font-medium text-primary hover:text-primary/80 transition-colors flex items-center gap-1">
                  View Cart <HiArrowRight className="w-3 h-3" />
                </button>
              </Link>
            </div>
            <div className="flex gap-2">
              <Link href="/cart" className="flex-1" onClick={hideToast}>
                <button className="w-full py-3 bg-primary text-white rounded-xl text-xs font-medium hover:bg-primary/90 transition-all shadow-lg shadow-primary/20">Checkout Now</button>
              </Link>
              <button onClick={hideToast} className="px-6 py-3 bg-white/10 text-white rounded-xl text-xs font-medium hover:bg-white/20 transition-all">Continue</button>
            </div>
          </div>
        </motion.div>
      )}
