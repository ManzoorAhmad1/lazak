...
export const StoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const cartTotal = cart.reduce((total, item) => {
    return total + ((item.price + (item.selectedVariant?.priceModifier || 0)) * item.quantity);
  }, 0);
  ...
  return (
    <StoreContext.Provider value={{ cart, favorites, addToCart, removeFromCart, updateQuantity, toggleFavorite, isFavorite, cartTotal, cartCount, favoriteCount, selectedProductForCart, openSelectionDrawer, closeSelectionDrawer, toast, showToast, hideToast }}>
      {children}
