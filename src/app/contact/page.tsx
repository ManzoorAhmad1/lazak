...
export default function ContactPage() {
  ...
  const contactItems = [
    { icon: HiEnvelope, title: 'Email Us', content: ['support@lazak.com', 'sales@lazak.com'], color: 'primary' },
    { icon: HiPhone, title: 'Call Us', content: ['+1 (800) 123-4567', 'Mon-Fri, 9am-6pm EST'], color: 'secondary' },
    { icon: HiMapPin, title: 'Based In', content: ['Los Angeles, CA', 'United States'], color: 'accent' },
      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-start">
          <motion.div {...sectionVariants} viewport={{ once: true }} className="space-y-12">
            ...
            <motion.div whileHover={{ scale: 1.01 }} className="w-full h-72 bg-muted/10 rounded-[32px] border border-muted/20 flex items-center justify-center overflow-hidden relative shadow-soft">
              <div className="absolute inset-0 bg-cover opacity-30 grayscale" style={{ backgroundImage: `url('https://images.unsplash.com/photo-1526778548025-fa2f459cd5c1?auto=format&fit=crop&q=80&w=1000')` }} />
              <div className="relative z-10 text-center space-y-3">
                <div className="w-16 h-16 bg-white/90 backdrop-blur-xl rounded-full flex items-center justify-center mx-auto shadow-medium"><HiMapPin className="w-8 h-8 text-primary" /></div>
                <p className="font-medium text-text-main">123 Tech Plaza, Silicon Valley</p>
              </div>
            </motion.div>
          </motion.div>
        ...
        <motion.section {...sectionVariants} viewport={{ once: true }} className="mt-40 max-w-4xl mx-auto space-y-12">
          ...
          <div className="space-y-4">
            {faqs.map((faq, i) => (
              <div key={i} className={`group border rounded-3xl transition-all duration-300 ${activeFaq === i ? 'bg-surface border-primary/20 shadow-medium' : 'bg-background border-muted/10 hover:border-muted/30'}`}>
                <button onClick={() => setActiveFaq(activeFaq === i ? null : i)} className="w-full flex items-center justify-between p-6 md:p-8 text-left">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${activeFaq === i ? 'bg-primary text-white' : 'bg-muted/5 text-primary group-hover:bg-primary/10'}`}><HiChatBubbleOvalLeft className="w-5 h-5" /></div>
                    <span className="text-lg md:text-xl font-medium text-text-main">{faq.q}</span>
                  </div>
                  <HiChevronDown className={`w-5 h-5 text-muted transition-transform duration-300 ${activeFaq === i ? 'rotate-180 text-primary' : ''}`} />
                </button>
                <AnimatePresence>
                  {activeFaq === i && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
                      <div className="px-6 md:px-8 pb-8 pt-0 ml-14"><p className="text-text-muted text-base md:text-lg leading-relaxed">{faq.a}</p></div>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
