...
import { HiStar, HiShoppingCart, HiHeart, HiShieldCheck, HiTruck, HiArrowPath, HiChevronRight, HiHome } from 'react-icons/hi2';
    ...
    <main className="pt-32 pb-20 bg-background min-h-screen">
      <div className="container mx-auto px-4 md:px-6">
        <nav className="flex items-center gap-2 text-sm text-text-muted mb-8">
          <Link href="/" className="hover:text-primary transition-colors flex items-center gap-1"><HiHome className="w-3 h-3" /> Home</Link>
          ...
          >
            <div className="space-y-4">
              <div className="flex flex-wrap gap-2 items-center">
                ...
                {product.hasFreeShipping && <Badge variant="secondary" className="rounded-none font-medium uppercase tracking-widest text-[10px]">Free Shipping</Badge>}
                {product.categories.map(cat => <Badge key={cat} variant="primary" className="bg-primary/5 rounded-none font-medium uppercase tracking-widest text-[10px]">{cat.replace('-', ' ')}</Badge>)}
              </div>
              <h1 className="text-4xl md:text-6xl font-medium tracking-tighter uppercase leading-none">{product.title}</h1>
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-0.5 text-accent">
                  {[...Array(5)].map((_, i) => <HiStar key={i} className={`w-4 h-4 ${i < Math.floor(product.rating) ? 'fill-current' : 'text-muted/20'}`} />)}
            ...
            <div className="space-y-10 pt-10 border-t border-muted/10">
              <div className="space-y-4">
                ...
                <div className="prose prose-sm max-w-none text-text-muted">
                  <p className="leading-relaxed text-base font-medium">{product.fullDescription || product.description}</p>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4 p-6 bg-muted/5 border border-muted/10">
                  <div className="flex items-center gap-3 text-primary"><HiTruck className="w-6 h-6" /><h4 className="font-medium uppercase tracking-widest text-sm">Shipping Information</h4></div>
                  <p className="text-sm text-text-muted leading-relaxed font-medium">Orders are processed within 24-48 hours. We offer free standard shipping on all orders over $50 within the continental USA. Typical delivery time is 3-5 business days.</p>
                </div>
                <div className="space-y-4 p-6 bg-muted/5 border border-muted/10">
                  <div className="flex items-center gap-3 text-primary"><HiArrowPath className="w-6 h-6" /><h4 className="font-medium uppercase tracking-widest text-sm">Return Policy</h4></div>
                  <p className="text-sm text-text-muted leading-relaxed font-medium">Not satisfied? We offer a 30-day hassle-free return policy. Return in original packaging for a full refund. No questions asked.</p>
                </div>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-8 border-t border-muted/10">
              {[{ icon: HiTruck, title: 'Free Shipping', sub: 'Orders Over $50' }, { icon: HiShieldCheck, title: '2 Year Warranty', sub: 'Full USA Coverage' }, { icon: HiArrowPath, title: '30 Day Returns', sub: 'Hassle-Free' }].map((item, i) => (
                <div key={i} className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-primary/10 rounded-none flex items-center justify-center text-primary"><item.icon className="w-6 h-6" /></div>
                  <div className="text-[10px]"><p className="font-medium uppercase tracking-widest">{item.title}</p><p className="text-text-muted font-medium">{item.sub}</p></div>
                </div>
              ))}
            </div>
          </motion.div>
